...
const {chalk} = require('../dist/bundle');
const program = require('commander');
const {clui} = require('../dist/bundle');
...
program
  ...
  .option('--root', 'Search in root path, not in node modules')
...
class ModCleanCLI {
  constructor() {
    ...
    // Display CLI header
    console.log(`\n${chalk.yellow.bold('DIARRHEA ')}${chalk.gray(` Version ${pkg.version}`)}\n`);
  ...
  start() {
    ...
    const options = {
      ...
      process(file, cb) {
        ...
        utils.confirm(`${chalk.gray(`(${self.stats.current}/${self.stats.total})`)}${name} ${chalk.gray(' - ')} ${chalk.yellow.bold('Delete File?')}`,
          (res) => {
